...
| **Admin** | `/admin` | Overview (health, quota, storage), Library (every title with its status, requester, playability and queue state, plus a drawer with retry, re-resolve, purge, mirror and blacklist actions), Requests (every user request with approve, inline deny and reopen; per-user quotas; auto-approve rules), Users, Settings, Filtering rules, Scrapers, Blacklist, Maintenance, Logs, Releases |
