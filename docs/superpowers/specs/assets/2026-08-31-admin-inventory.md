...
- [ ] Overview: TorBox Usage card (torrent count, total size, plan, per-state badges) -> `refreshOverview()` / `GET /ui/api/torbox-usage` -> planned home: Overview (out of scope for Task 6; the TorBox library stat tile covers count + size only, from `/ui/api/torbox-list` not `/ui/api/torbox-usage`) (Task 12 discharge ruling: will not be ported - citing the Task 6 note above: the TorBox library stat tile already covers count + size from a different endpoint, and no task in this plan implements `/ui/api/torbox-usage`)
...
- [ ] Requests: retry a failed request -> `POST /ui/api/requests/<id>/retry`, already wired in `frontend/src/pages/Requests.tsx` (`api.retryRequest`, outside this inventory's two named sources but confirmed live) -> planned home: Requests (Task 12 discharge ruling: will not be ported - citing the note above: the action is already live and user-facing via `frontend/src/pages/Requests.tsx`'s retry button; confirmed `frontend/src/pages/admin/Requests.tsx` has no duplicate retry control, and none is needed)
...
- [x] Maintenance: force strm rescan (topbar circular-arrow icon) -> `forceRescan()` / `POST /ui/strm-rescan` -> planned home: Maintenance (placement chosen by inventory - lives in the global topbar, not the Maintenance tab-pane, but is functionally a maintenance action) (Task 12 fix round: controller ruling - port it; added as an "Force strm rescan" action button in the Library group of `frontend/src/pages/admin/Maintenance.tsx`, `api.maintenanceStrmRescan()` -> `POST /ui/strm-rescan`, no confirm() since the Jinja control had none)
- [ ] Maintenance: manual release search by IMDB id/season/episode + candidate list (cached badge, season-pack badge, size, seeders) -> `doSearch()` / `POST /ui/api/search-candidates` -> planned home: Maintenance (placement chosen by inventory - no live control in the current 8-tab markup reaches this, see Findings; kept because it is the only path to Add magnet, which the map places here) (Task 12 discharge ruling: will not be ported - `frontend/src/pages/admin/Maintenance.tsx`'s `QuickActionCard` doc comment and the matching comment in `frontend/src/api.ts` (around the `maintenanceAddMagnet` block) explicitly rule the search-candidates browse-and-select UI "out of scope for this tab"; Add magnet, the line below, is ticked and covers the terminal action via direct magnet/URL input instead)
...
Section ruling: these are routes, not controls; no UI ever reached them, each line's
(orphan route, verify before porting) marker is its disposition, and the routes remain
server-side untouched. Discharged as a section.
