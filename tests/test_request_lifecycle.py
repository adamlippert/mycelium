...
def test_lazy_season_stores_null_source_when_the_release_name_has_no_tag(monkeypatch):
    """A release name that carries no detectable source tag must store NULL,
    not fall back to the scraper name (source="torrentio" here)."""
    import blacklist
    import debrid
    import strm_generator
    pack = streams_mod.Stream(
        name="The.Rookie.S05.1080p.DDP5.1.x264", title="The.Rookie.S05.1080p",
        info_hash="a" * 40, quality="1080p", seeders=10, size_gb=5.0,
        is_season_pack=True, source="torrentio",
    )
    monkeypatch.setattr(processor, "_fetch_season_candidates", lambda *a, **k: [pack])
    ...
    assert ok is True
    assert calls and calls[0]["source"] is None
